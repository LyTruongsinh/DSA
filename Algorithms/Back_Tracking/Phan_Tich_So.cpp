...
void partitionNumber(int n, vector<int> &current, int index) {
    ...
    // ví dụ đẩy vào 3 thì nó chỉ có từ 3 trở xuống nhưng tổng các số tiếp theo khi đẩy vào vẫn phải thỏa mãn tổng của nó = n - 3 = 2
    // ví dụ đẩy vào 2 thì nó chỉ có từ 2 trở xuống nếu đẩy vào cái lớn > 2 thì nó sẽ lặp lại nhưng tổng các số tiếp theo khi đẩy vào vẫn phải thỏa mãn tổng của nó = n - 2 = 3 
    for (int i = index; i >= 1; --i) {
        if(i <= n) { // những số lớn hơn số cần phân tích thì bỏ qua
            current.push_back(i);
