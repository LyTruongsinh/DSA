...
bool bellman_ford(int s) {
    dist.assign(n + 1, INF);   // Gán tất cả các đỉnh là vô cùng (chưa biết đường đi)
    parent.assign(n + 1, -1);  // Gán cha của tất cả các đỉnh là -1
    dist[s] = 0; // Đặt khoảng cách từ đỉnh s đến chính nó là 0

    for (int i = 1; i <= n - 1; i++) {  // Lặp lại n-1 lần để đảm bảo tính chính xác
        for (auto edge : edges) {  // Duyệt tất cả các cạnh
            if (dist[edge.u] != INF && dist[edge.u] + edge.w < dist[edge.v]) {  // Nếu có thể cập nhật
...
vector<vector<int>> floyd_warshall() {
   ...
   for (int i = 1; i <= n; ++i) dist_fw[i][i] = 0;
   for (auto edge : edges)
      ...
      for (int i = 1; i <= n; ++i) {
         for (int j = 1; j <= n; ++j) {
            if (dist_fw[i][k] < INF && dist_fw[k][j] < INF) {
               dist_fw[i][j] =
                   min(dist_fw[i][j], dist_fw[i][k] + dist_fw[k][j]);
            }
         }
      }
   }
